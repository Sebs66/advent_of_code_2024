...
pub fn calculate_movements(
    ...
    check_next_obstacles(&obstacles_positions, direction, position);
...
fn check_next_obstacles(
    ...
    let (mut is_an_obstacle, _, mut new_direction, mut new_position) =
        find_next_obstacles(obstacles_positions, position, next_direction);
    while is_an_obstacle {
        (is_an_obstacle, _, new_direction, new_position) =
            find_next_obstacles(obstacles_positions, new_position, new_direction);
    }
}
...
fn find_next_obstacles(
    ...
    evaluate_candidate_position_for_obstacle(position, direction, obstacles_positions);
    let (is_obstacle, obstacle_coords, next_direction, new_position) =
        evaluate_next_obstacle(position, direction, obstacles_positions);

    return (is_obstacle, obstacle_coords, next_direction, new_position);
}
...
fn evaluate_candidate_position_for_obstacle(
    ...
    let next_direction_mapper = init_next_direction();
}
